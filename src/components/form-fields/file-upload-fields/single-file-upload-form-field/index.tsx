...
export const SingleFileUploadFormField = (props: any) => {
  const {
    name,
    required,
    accept = {
  ...
  return (
    <>
      ...
      <label htmlFor={name}>
        ...
        >
          {!!attachedFile ? (
            ...
            <>
              <CommonFolderIcon />
              <BodyText marginTop={1} marginBottom={1} color="text.disabled">
                Drag & Drop File Here OR
              </BodyText>
              <AddNewItemButton onClick={handleBrowseClick} hasEndIcon>
                Browse Files
              </AddNewItemButton>
            </>
          )}
      ...
      {!!errors?.[name]?.message && !!!getValues(name)?.name && (
...
export default SingleFileUploadFormField;
